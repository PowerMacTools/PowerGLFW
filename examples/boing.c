...
void DrawBoingBallBand(GLfloat long_lo, GLfloat long_hi) {
	...
	for(lat_deg = 0; lat_deg <= (360 - STEP_LATITUDE);
		 ...
		 */
		if(colorToggle) glColor3f(0.8f, 0.1f, 0.1f);
		else
...
int main(void) {
	...
	window =
		glfwCreateWindow(800, 600, "Boing (classic Amiga demo)", NULL, NULL);
	if(!window) {
		glfwTerminate();
		exit(EXIT_FAILURE);
	}

#ifdef glfwExtensionSupported
